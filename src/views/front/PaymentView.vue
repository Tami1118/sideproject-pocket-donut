              ...
              <table class="table table-borderless">
                ...
                <tbody class="border-top">
                  <tr v-for="item in cart.carts" :key="item.id">
                    ...
                    <td width="30%">{{ item.product.title }}</td>
                    <td width="10%" class="text-end">{{ item.qty }}</td>
                    <td width="20%" class="text-end">{{ item.product.price * item.qty }}</td>
                  </tr>
                </tbody>
                <tfoot class="border-top">
                  ...
                  <tr>
                    <td>優惠折扣</td>
                    <td colspan="3" class="text-end">- {{ total - final_total }}</td>
                  </tr>
                  <tr class="border-top">
                    <td>訂單總額</td>
                    <td colspan="3" class="text-end text-primary fw-bold fs-4">
                      NT$ {{ final_total }}
                    </td>
                  </tr>
                </tfoot>
            ...
            <div class="order_form p-4 bg_gray rounded-4">
              ...
              <VForm v-slot="{ errors }" @submit="onSubmit">
                ...
                <div class="p-3 d-flex justify-content-between mt-3">
                  ...
                  <button type="submit" class="btn btn-primary" style="width: 6rem" @click="createOrder">下一步</button>
                </div>
              </VForm>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    createOrder(){
      ...
      this.$http.post(url, {data:this.data})
        .then(res => {
          console.log(res)
          this.data = {
            user: {
